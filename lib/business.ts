...
export type SubscriptionFrequency = 'monthly' | 'quarterly' | 'semiannual' | 'annual' | 'custom';

export function billingMonths(frequency: SubscriptionFrequency, customIntervalMonths?: number | null) {
  ...
  if (frequency === 'custom') {
    if (!customIntervalMonths || customIntervalMonths < 1) throw new Error('Intervalo personalizado inválido.');
...
export function addMonthsClamped(dateValue: string, months: number, preferredDay?: number) {
  ...
  const targetDay = Math.min(preferredDay ?? day, daysInMonth(nextYear, nextMonthIndex));
...
export function nextSubscriptionDate(input: {
  ...
  return addMonthsClamped(input.currentDate, billingMonths(input.frequency, input.customIntervalMonths), input.billingDay);
}

export function billingPeriod(dateValue: string) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateValue)) throw new Error('Data de cobrança inválida.');
...
export function subscriptionDatesThroughHorizon(input: {
  ...
  let guard = 0;
  while (nextDate <= input.horizon && (!input.endDate || nextDate <= input.endDate) && guard < 240) {
    dates.push(nextDate);
...
export function initialChargeStatus(dueDate: string, today: string): 'scheduled' | 'pending' {
  return dueDate <= today ? 'pending' : 'scheduled';
}

export function shouldMarkOverdue(dueDate: string, today: string, status: string) {
  return status === 'pending' && dueDate < today;
}

export function domainTaskKey(domainId: string, expirationDate: string, threshold: number) {
...
export function crossedDomainThresholds(expirationDate: string, today: string, thresholds: readonly number[]) {
  const days = differenceInCalendarDays(expirationDate, today);
  return thresholds.filter((threshold) => days <= threshold);
}

export function nextPostSaleDate(lastContactDate: string, frequencyMonths: number) {
...
export function moneyToMinorUnits(value: string | number) {
  const normalized = typeof value === 'number' ? value.toFixed(2) : value.trim().replace(/\./g, '').replace(',', '.');
...
export function pipelineTransition(input: { isWon: boolean; isLost: boolean; now?: Date; lostReason?: string | null }) {
