...
type SetInteractionResultInput = z.infer<typeof agentSetInteractionResultSchema>;
...
async function completedReplay<T>(input: {
  ...
  if (existing.operation !== input.operation || existing.requestHash !== requestHash) {
    throw new AgentApiError(
...
function activity(input: {
  ...
  return db.insert(activities).values({ ...input, metadata: input.metadata || {} });
}
...
async function ownedCompany(ownerUserId: string, companyId: string) {
  const [company] = await db
    ...
    .limit(1);
  if (!company) throw new AgentApiError('AGENT_LEAD_NOT_FOUND', 'Lead não encontrado.', 404);
  return company;
}

async function ensureNoDuplicateLead(ownerUserId: string, input: CreateLeadInput, ignoredId?: string) {
  const rows = await db
    .select({
      id: companies.id,
      name: companies.name,
    ...
    .from(companies)
    .where(and(eq(companies.ownerUserId, ownerUserId), isNull(companies.archivedAt)));
...
export async function createAgentLead(
  ...
  const prior = await completedReplay<{ id: string; createdAt: Date; updatedAt: Date }>({ actor, idempotencyKey, operation: 'crm_create_lead', request: input });
  if (prior) return prior;
  await ensureNoDuplicateLead(actor.ownerUserId, input);
  if (input.referredByCompanyId) await ownedCompany(actor.ownerUserId, input.referredByCompanyId);
  const id = randomUUID();
  const now = new Date();
  ...
  return idempotentWrite({
    ...
    request: input,
    response,
    audit: { entityType: 'companies', entityId: id, changes: { before: null, after: input } },
    run: async (reserve) => {
      await db.batch([
        reserve,
        db.insert(companies).values({ id, ownerUserId: actor.ownerUserId, ...input, createdAt: now, updatedAt: now }),
        activity({
...
export async function updateAgentLead(
  ...
  const prior = await completedReplay<{ id: string; updatedAt: Date }>({ actor, idempotencyKey, operation: 'crm_update_lead', request: { leadId, ...input } });
  if (prior) return prior;
  ...
  if (changes.name || changes.city || changes.document || changes.email || changes.phone || changes.whatsapp || changes.website) {
    await ensureNoDuplicateLead(actor.ownerUserId, { ...current, ...changes } as CreateLeadInput, leadId);
  }
  if (changes.referredByCompanyId) await ownedCompany(actor.ownerUserId, changes.referredByCompanyId);
  const now = new Date();
  ...
  return idempotentWrite({
    ...
    audit: {
      entityType: 'companies',
      entityId: leadId,
      changes: { before: Object.fromEntries(Object.keys(changes).map((key) => [key, current[key as keyof typeof current]])), after: changes },
    },
    run: async (reserve) => {
      const updatedLead = db.$with('updated_lead').as(
        db.update(companies).set({ ...changes, updatedAt: now }).where(and(
      ...
      await db.batch([
        reserve,
        db.with(updatedLead).select({ concurrencyGuard: versionGuard(updatedLead) }).from(sql`(select 1) as concurrency_guard_source`),
        activity({
          ownerUserId: actor.ownerUserId,
          companyId: leadId,
          entityType: 'companies',
...
export async function setAgentLeadStage(
  ...
  const prior = await completedReplay<{ id: string; prospectingStatus: string; updatedAt: Date }>({ actor, idempotencyKey, operation: 'crm_set_lead_stage', request: { leadId, ...input } });
  if (prior) return prior;
  const current = await ownedCompany(actor.ownerUserId, leadId);
  assertExpected(current.updatedAt, input.expectedUpdatedAt);
  const now = new Date();
  const response = { id: leadId, prospectingStatus: input.prospectingStatus, updatedAt: now };
  return idempotentWrite({
    actor,
    idempotencyKey,
    ...
    audit: {
      entityType: 'companies',
      entityId: leadId,
      changes: { before: { prospectingStatus: current.prospectingStatus }, after: { prospectingStatus: input.prospectingStatus, reason: input.reason } },
    },
    run: async (reserve) => {
      const updatedLeadStage = db.$with('updated_lead_stage').as(
        db.update(companies).set({ prospectingStatus: input.prospectingStatus, updatedAt: now }).where(and(
          eq(companies.id, leadId),
          eq(companies.ownerUserId, actor.ownerUserId),
      ...
      await db.batch([
        reserve,
        db.with(updatedLeadStage).select({ concurrencyGuard: versionGuard(updatedLeadStage) }).from(sql`(select 1) as concurrency_guard_source`),
        activity({
          ownerUserId: actor.ownerUserId,
          companyId: leadId,
          entityType: 'companies',
          ...
          metadata: { previousProspectingStatus: current.prospectingStatus, prospectingStatus: input.prospectingStatus, reason: input.reason, source: 'agent' },
        }),
      ]);
    },
  });
...
export async function moveAgentOpportunity(
  ...
  const prior = await completedReplay<{ id: string; pipelineStageId: string; stage: string; updatedAt: Date }>({ actor, idempotencyKey, operation: 'crm_move_opportunity', request: { opportunityId, ...input } });
  if (prior) return prior;
  const [[opportunity], [stage]] = await Promise.all([
    db.select().from(opportunities).where(and(
  ...
  if (!opportunity) throw new AgentApiError('AGENT_OPPORTUNITY_NOT_FOUND', 'Oportunidade não encontrada.', 404);
  if (!stage) throw new AgentApiError('AGENT_STAGE_NOT_FOUND', 'Etapa comercial não encontrada.', 404);
  ...
  const now = new Date();
  const response = { id: opportunityId, pipelineStageId: stage.id, stage: stage.slug, updatedAt: now };
  return idempotentWrite({
    actor,
    idempotencyKey,
    ...
    audit: {
      ...
      changes: { before: { pipelineStageId: opportunity.pipelineStageId }, after: { pipelineStageId: stage.id, reason: input.reason, lostReason: input.lostReason } },
    },
    run: async (reserve) => {
      ...
      const statements = [
        reserve,
        db.with(updatedOpportunity).select({ concurrencyGuard: versionGuard(updatedOpportunity) }).from(sql`(select 1) as concurrency_guard_source`),
        ...
        activity({
          ...
          metadata: { stageId: stage.id, stage: stage.slug, reason: input.reason, source: 'agent' },
        }),
      ] as const;
...
export async function createAgentInteraction(
  ...
  const prior = await completedReplay<{ id: string; followUpId: string | null; occurredAt: Date; updatedAt: Date }>({ actor, idempotencyKey, operation: 'crm_create_interaction', request: input });
  if (prior) return prior;
  const company = await ownedCompany(actor.ownerUserId, input.companyId);
  if (input.opportunityId) {
    const [opportunity] = await db.select({ id: opportunities.id }).from(opportunities).where(and(
      eq(opportunities.id, input.opportunityId),
      eq(opportunities.ownerUserId, actor.ownerUserId),
      eq(opportunities.companyId, input.companyId),
      isNull(opportunities.archivedAt),
    )).limit(1);
    if (!opportunity) throw new AgentApiError('AGENT_OPPORTUNITY_NOT_FOUND', 'Oportunidade vinculada não encontrada.', 404);
  }
  const id = randomUUID();
  const occurredAt = input.occurredAt || new Date();
  const now = new Date();
  const followUpId = input.nextAction && input.nextActionAt ? randomUUID() : null;
  const nextContactAt = company.contactFrequencyMonths
    ? dateAtNoon(nextPostSaleDate(occurredAt.toISOString().slice(0, 10), company.contactFrequencyMonths))
  ...
  return idempotentWrite({
    ...
    request: input,
    response,
    audit: { entityType: 'interactions', entityId: id, changes: { before: null, after: input } },
    run: async (reserve) => {
      const core = [
        reserve,
        ...
        db.update(companies).set({
          ...
          updatedAt: now,
        }).where(and(eq(companies.id, input.companyId), eq(companies.ownerUserId, actor.ownerUserId))),
        activity({
          ownerUserId: actor.ownerUserId,
          companyId: input.companyId,
...
export async function setAgentInteractionResult(
  ...
  const prior = await completedReplay<{ id: string; followUpId: string | null; updatedAt: Date }>({ actor, idempotencyKey, operation: 'crm_set_interaction_result', request: { interactionId, ...input } });
  if (prior) return prior;
  const [current] = await db.select().from(interactions).where(and(
    eq(interactions.id, interactionId),
    eq(interactions.ownerUserId, actor.ownerUserId),
  )).limit(1);
  if (!current) throw new AgentApiError('AGENT_INTERACTION_NOT_FOUND', 'Contato não encontrado.', 404);
  assertExpected(current.updatedAt, input.expectedUpdatedAt);
  const now = new Date();
  const followUpId = input.nextAction && input.nextActionAt ? randomUUID() : null;
  const response = { id: interactionId, followUpId, updatedAt: now };
  const after = { result: input.result, notes: input.notes, nextAction: input.nextAction, nextActionAt: input.nextActionAt };
  return idempotentWrite({
    actor,
    idempotencyKey,
    ...
    response,
    audit: { entityType: 'interactions', entityId: interactionId, changes: { before: { result: current.result, notes: current.notes, nextAction: current.nextAction, nextActionAt: current.nextActionAt }, after } },
    run: async (reserve) => {
      const updatedInteraction = db.$with('updated_interaction').as(
        db.update(interactions).set({ ...after, updatedAt: now }).where(and(
          eq(interactions.id, interactionId),
          eq(interactions.ownerUserId, actor.ownerUserId),
      ...
      const core = [
        reserve,
        db.with(updatedInteraction).select({ concurrencyGuard: versionGuard(updatedInteraction) }).from(sql`(select 1) as concurrency_guard_source`),
        activity({
          ownerUserId: actor.ownerUserId,
...
export async function createAgentFollowUp(
  ...
  const prior = await completedReplay<{ id: string; dueAt: Date; createdAt: Date; updatedAt: Date }>({ actor, idempotencyKey, operation: 'crm_create_follow_up', request: input });
  if (prior) return prior;
  ...
  return idempotentWrite({
    ...
    request: input,
    response,
    audit: { entityType: 'tasks', entityId: id, changes: { before: null, after: input } },
    run: async (reserve) => {
      await db.batch([
        reserve,
        db.insert(tasks).values({
        ...
        db.update(companies).set({ nextAction: input.title, nextActionAt: input.dueAt, updatedAt: now }).where(and(
...
export async function createAgentReferral(
  ...
  const prior = await completedReplay<{ id: string; createdAt: Date; updatedAt: Date }>({ actor, idempotencyKey, operation: 'crm_create_referral', request: input });
  if (prior) return prior;
  await Promise.all([
    ownedCompany(actor.ownerUserId, input.referrerCompanyId),
    ownedCompany(actor.ownerUserId, input.referredCompanyId),
  ]);
  const [duplicate] = await db.select({ id: referrals.id }).from(referrals).where(and(
  ...
  if (duplicate) throw new AgentApiError('AGENT_DUPLICATE_REFERRAL', 'Esta indicação já está registrada.', 409, { referralId: duplicate.id });
  const id = randomUUID();
  const now = new Date();
  const response = { id, createdAt: now, updatedAt: now };
  return idempotentWrite({
    ...
    request: input,
    response,
    audit: { entityType: 'referrals', entityId: id, changes: { before: null, after: input } },
    run: async (reserve) => {
      await db.batch([
        reserve,
        ...
        db.update(companies).set({ referredByCompanyId: input.referrerCompanyId, updatedAt: now }).where(and(
        ...
        activity({
          ...
          metadata: { referrerCompanyId: input.referrerCompanyId, source: 'agent' },
        }),
      ]);
    },
  });
...
export async function upsertAgentDigitalAnalysis(
  ...
  const prior = await completedReplay<{ id: string; leadId: string; leadScore: number; scoreLevel: string; updatedAt: Date }>({ actor, idempotencyKey, operation: 'crm_upsert_digital_analysis', request: { leadId, ...input } });
  if (prior) return prior;
  const [company, [existing], customRules] = await Promise.all([
    ownedCompany(actor.ownerUserId, leadId),
    db.select().from(digitalAnalyses).where(and(
      eq(digitalAnalyses.ownerUserId, actor.ownerUserId),
      eq(digitalAnalyses.companyId, leadId),
    )).limit(1),
    db.select().from(leadScoreRules).where(eq(leadScoreRules.ownerUserId, actor.ownerUserId)),
  ]);
  if (existing && !input.expectedUpdatedAt) {
    throw new AgentApiError('AGENT_VERSION_REQUIRED', 'expectedUpdatedAt é obrigatório para atualizar uma análise existente.', 409, { currentUpdatedAt: existing.updatedAt.toISOString() });
  }
  if (existing && input.expectedUpdatedAt) assertExpected(existing.updatedAt, input.expectedUpdatedAt);
  const { expectedUpdatedAt, ...analysis } = input;
  const merged = { ...existing, ...analysis };
  const score = analysis.scoreOverride !== null && analysis.scoreOverride !== undefined
    ? {
        score: analysis.scoreOverride,
        level: leadScoreLevel(analysis.scoreOverride),
        breakdown: [{ ruleKey: 'manual_override', label: 'Pontuação informada manualmente', points: analysis.scoreOverride }],
      }
    : calculateLeadScore(company, merged, customRules.length ? customRules : DEFAULT_LEAD_SCORE_RULES);
  ...
  const response = { id, leadId, leadScore: score.score, scoreLevel: score.level, updatedAt: now };
  return idempotentWrite({
    actor,
    idempotencyKey,
    ...
    request: { leadId, ...input },
    response,
    audit: { entityType: 'digital_analyses', entityId: id, changes: { before: existing || null, after: values } },
    run: async (reserve) => {
      if (existing) {
        const updatedAnalysis = db.$with('updated_analysis').as(
          db.update(digitalAnalyses).set(values).where(and(
        ...
        await db.batch([
          reserve,
          db.with(updatedAnalysis).select({ concurrencyGuard: versionGuard(updatedAnalysis) }).from(sql`(select 1) as concurrency_guard_source`),
          activity({
            ownerUserId: actor.ownerUserId,
            companyId: leadId,
