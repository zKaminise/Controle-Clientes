...
const money = (name: string) =>
  numeric(name, { precision: 12, scale: 2 }).default('0').notNull();
...
export const prospectingStatus = pgEnum('prospecting_status', [
  'NOVO_LEAD',
  'PESQUISANDO',
  'PRONTO_PARA_CONTATO',
  'CONTATO_WHATSAPP',
...
export const siteAnalysisStatus = pgEnum('site_analysis_status', [
  'SEM_SITE',
  'SITE_RUIM',
  'SITE_DEFASADO',
  'SITE_MEDIANO',
...
export const leadScoreLevel = pgEnum('lead_score_level', [
  'BAIXA',
  'MEDIA',
  'ALTA',
  'MUITO_ALTA',
...
export const domainResponsibility = pgEnum('domain_responsibility', [
  'me',
  'client',
  'third_party',
]);
...
// Better Auth JWT + OAuth 2.1 provider tables used by the remote MCP server.
...
export const oauthClients = pgTable(
  ...
  (table) => [
    uniqueIndex('oauth_clients_client_id_uq').on(table.clientId),
...
export const oauthRefreshTokens = pgTable(
  ...
  (table) => [
    uniqueIndex('oauth_refresh_tokens_token_uq').on(table.token),
...
export const oauthAccessTokens = pgTable(
  ...
  (table) => [
    uniqueIndex('oauth_access_tokens_token_uq').on(table.token),
...
export const agentAuditLogs = pgTable(
  'agent_audit_logs',
  {
    ...
    toolName: varchar('tool_name', { length: 96 }),
  ...
  (table) => [
    ...
    index('agent_audit_logs_oauth_client_date_idx').on(
      table.oauthClientId,
      table.createdAt,
    ),
  ],
...
export const companies = pgTable(
  'companies',
  {
    ...
    prospectingStatus: prospectingStatus('prospecting_status')
      .default('NOVO_LEAD')
      .notNull(),
    referredByCompanyId: uuid('referred_by_company_id').references(
  ...
  (table) => [
    uniqueIndex('companies_owner_name_city_uq').on(
      table.ownerUserId,
      table.name,
      table.city,
    ),
    uniqueIndex('companies_owner_document_uq').on(
      table.ownerUserId,
      table.document,
    ),
    index('companies_owner_lifecycle_idx').on(
      table.ownerUserId,
      table.lifecycleStatus,
      table.archivedAt,
    ),
    index('companies_owner_prospecting_idx').on(
      table.ownerUserId,
      table.prospectingStatus,
      table.archivedAt,
    ),
    index('companies_next_contact_idx').on(
      table.ownerUserId,
      table.nextContactAt,
    ),
    check(
      'companies_contact_frequency_positive',
      sql`${table.contactFrequencyMonths} is null or ${table.contactFrequencyMonths} > 0`,
    ),
  ],
);

export const digitalAnalyses = pgTable(
  'digital_analyses',
  {
    id: id(),
    ownerUserId: uuid('owner_user_id')
    ...
    mobileQuality: integer('mobile_quality'),
    speedQuality: integer('speed_quality'),
    designQuality: integer('design_quality'),
    valuePropositionQuality: integer('value_proposition_quality'),
    ctaQuality: integer('cta_quality'),
    hasWhatsappIntegration: boolean('has_whatsapp_integration'),
    hasBasicSeo: boolean('has_basic_seo'),
    hasHttps: boolean('has_https'),
    hasBrokenLinks: boolean('has_broken_links'),
    hasActiveDigitalPresence: boolean('has_active_digital_presence'),
    issues: text('issues'),
    opportunities: text('opportunities'),
    leadScore: integer('lead_score').default(0).notNull(),
    scoreOverride: integer('score_override'),
    scoreLevel: leadScoreLevel('score_level').default('BAIXA').notNull(),
    priority: leadScoreLevel('priority').default('BAIXA').notNull(),
    scoreBreakdown: jsonb('score_breakdown')
      .$type<Array<{ ruleKey: string; label: string; points: number }>>()
      .default([])
      .notNull(),
  ...
  (table) => [
    uniqueIndex('digital_analyses_owner_company_uq').on(
      table.ownerUserId,
      table.companyId,
    ),
    index('digital_analyses_owner_score_idx').on(
      table.ownerUserId,
      table.leadScore,
    ),
    index('digital_analyses_owner_site_status_idx').on(
      table.ownerUserId,
      table.siteStatus,
    ),
    check(
      'digital_analyses_score_range',
      sql`${table.leadScore} between 0 and 100`,
    ),
    check(
      'digital_analyses_score_override_range',
      sql`${table.scoreOverride} is null or ${table.scoreOverride} between 0 and 100`,
    ),
    check(
      'digital_analyses_overall_range',
      sql`${table.overallQuality} is null or ${table.overallQuality} between 0 and 5`,
    ),
    check(
      'digital_analyses_mobile_range',
      sql`${table.mobileQuality} is null or ${table.mobileQuality} between 0 and 5`,
    ),
    check(
      'digital_analyses_speed_range',
      sql`${table.speedQuality} is null or ${table.speedQuality} between 0 and 5`,
    ),
    check(
      'digital_analyses_design_range',
      sql`${table.designQuality} is null or ${table.designQuality} between 0 and 5`,
    ),
    check(
      'digital_analyses_value_proposition_range',
      sql`${table.valuePropositionQuality} is null or ${table.valuePropositionQuality} between 0 and 5`,
    ),
    check(
      'digital_analyses_cta_range',
      sql`${table.ctaQuality} is null or ${table.ctaQuality} between 0 and 5`,
    ),
  ],
);
...
export const contacts = pgTable(
  'contacts',
  {
    ...
    isFinancialContact: boolean('is_financial_contact')
      .default(false)
      .notNull(),
    notes: text('notes'),
    archivedAt: timestamp('archived_at', { withTimezone: true }),
...
export const pipelineStages = pgTable(
  ...
  (table) => [
    uniqueIndex('pipeline_stages_owner_slug_uq').on(
      table.ownerUserId,
      table.slug,
    ),
    uniqueIndex('pipeline_stages_owner_position_uq').on(
      table.ownerUserId,
      table.position,
    ),
    check(
      'pipeline_stages_terminal_exclusive',
      sql`not (${table.isWon} and ${table.isLost})`,
    ),
  ],
);
...
export const opportunities = pgTable(
  ...
  (table) => [
    index('opportunities_stage_idx').on(
      table.ownerUserId,
      table.pipelineStageId,
      table.archivedAt,
    ),
    index('opportunities_next_action_idx').on(
      table.ownerUserId,
      table.nextActionAt,
    ),
...
export const pipelineHistory = pgTable(
  ...
  (table) => [
    index('pipeline_history_opportunity_date_idx').on(
      table.opportunityId,
      table.changedAt,
    ),
    index('pipeline_history_owner_date_idx').on(
      table.ownerUserId,
      table.changedAt,
    ),
  ],
...
export const referrals = pgTable(
  'referrals',
  {
    ...
    ownerUserId: uuid('owner_user_id')
      .notNull()
      .references(() => users.id, { onDelete: 'restrict' }),
    referrerCompanyId: uuid('referrer_company_id')
      .notNull()
      .references(() => companies.id, { onDelete: 'restrict' }),
    referredCompanyId: uuid('referred_company_id')
      .notNull()
      .references(() => companies.id, { onDelete: 'cascade' }),
    status: referralStatus('status').default('PENDENTE').notNull(),
    notes: text('notes'),
  ...
  (table) => [
    uniqueIndex('referrals_owner_pair_uq').on(
      table.ownerUserId,
      table.referrerCompanyId,
      table.referredCompanyId,
    ),
    index('referrals_referrer_idx').on(table.referrerCompanyId, table.status),
    index('referrals_referred_idx').on(table.referredCompanyId, table.status),
    check(
      'referrals_not_self',
...
export const leadScoreRules = pgTable(
  ...
  (table) => [
    uniqueIndex('lead_score_rules_owner_key_uq').on(
      table.ownerUserId,
      table.ruleKey,
    ),
    uniqueIndex('lead_score_rules_owner_position_uq').on(
      table.ownerUserId,
      table.position,
    ),
    check(
      'lead_score_rules_points_range',
...
export const projects = pgTable(
  ...
  (table) => [
    index('projects_company_idx').on(table.companyId, table.archivedAt),
  ],
);
...
export const technologies = pgTable(
  ...
  (table) => [
    uniqueIndex('technologies_owner_name_uq').on(table.ownerUserId, table.name),
  ],
);
...
export const domains = pgTable(
  'domains',
  {
    ...
    responsibility: domainResponsibility('responsibility')
      .default('client')
      .notNull(),
    registeredUnderMyAccount: boolean('registered_under_my_account')
      .default(false)
      .notNull(),
    registeredUnderClientDocument: boolean('registered_under_client_document')
      .default(false)
      .notNull(),
  ...
  (table) => [
    ...
    index('domains_expiration_idx').on(
      table.ownerUserId,
...
export const hostingServices = pgTable(
  ...
  (table) => [
    index('hosting_company_idx').on(table.companyId, table.archivedAt),
  ],
);

export const emailServices = pgTable(
  'email_services',
  {
    id: id(),
    ownerUserId: uuid('owner_user_id')
  ...
  (table) => [
    index('email_services_company_idx').on(table.companyId, table.archivedAt),
  ],
);
...
export const services = pgTable(
  ...
  (table) => [
    uniqueIndex('services_owner_name_uq').on(table.ownerUserId, table.name),
  ],
);
...
export const subscriptions = pgTable(
  ...
  (table) => [
    index('subscriptions_next_charge_idx').on(
      table.ownerUserId,
      table.status,
    ...
    check(
      ...
      sql`${table.customIntervalMonths} is null or ${table.customIntervalMonths} > 0`,
    ),
  ],
);
...
export const charges = pgTable(
  ...
  (table) => [
    uniqueIndex('charges_subscription_period_uq').on(
      table.subscriptionId,
      table.billingPeriod,
    ),
    index('charges_due_status_idx').on(
      table.ownerUserId,
      table.dueDate,
      table.status,
    ),
...
export const proposals = pgTable(
  ...
  (table) => [
    ...
    index('proposals_followup_idx').on(
      table.ownerUserId,
      table.status,
      table.sentAt,
    ),
...
export const meetings = pgTable(
  ...
  (table) => [
    index('meetings_schedule_idx').on(
      table.ownerUserId,
      table.meetingAt,
      table.archivedAt,
    ),
...
export const tasks = pgTable(
  ...
  (table) => [
    uniqueIndex('tasks_owner_idempotency_uq').on(
      table.ownerUserId,
      table.idempotencyKey,
    ),
    index('tasks_due_status_idx').on(
      table.ownerUserId,
      table.dueAt,
      table.status,
    ),
...
export const messageTemplates = pgTable(
  ...
  (table) => [
    uniqueIndex('message_templates_owner_name_uq').on(
      table.ownerUserId,
      table.name,
    ),
  ],
...
export const messageLogs = pgTable(
  ...
  (table) => [
    index('message_logs_owner_date_idx').on(table.ownerUserId, table.createdAt),
  ],
);
...
export const notifications = pgTable(
  ...
  (table) => [
    uniqueIndex('notifications_user_idempotency_uq').on(
      table.userId,
      table.idempotencyKey,
    ),
...
export const tags = pgTable(
  ...
  (table) => [
    uniqueIndex('tags_owner_name_uq').on(table.ownerUserId, table.name),
  ],
);
...
export const companyTags = pgTable(
  ...
  (table) => [
    primaryKey({
      columns: [table.companyId, table.tagId],
      name: 'company_tags_pk',
    }),
  ],
);
...
export const settings = pgTable(
  'settings',
  {
    ...
    domainAlertDays: jsonb('domain_alert_days')
      .$type<number[]>()
      .default([60, 30, 15, 7, 3, 0])
      .notNull(),
...
export const automationRuns = pgTable(
  ...
  (table) => [
    index('automation_runs_job_date_idx').on(table.jobName, table.startedAt),
  ],
);
